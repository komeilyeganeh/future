...
export default function ModularDesignModules() {
  ...
  return (
    <section id="modules" className="bg-[#f8faf9]">
      <div className="mx-auto max-w-7xl px-5 py-24 md:px-8 md:py-32">
        ...
        <div className="flex flex-col gap-8 md:flex-row md:items-end md:justify-between">
          ...
          <div className="relative w-full md:max-w-xs">
            <Search
              size={16}
              className="absolute left-4 top-1/2 -translate-y-1/2 text-[#71838a]"
            />
            ...
            <input
              ...
              className="h-12 w-full rounded-full border border-[#082f3a]/8 bg-white pl-11 pr-5 text-xs text-[#082f3a] outline-none transition focus:border-[#082f3a]/20"
            />
          </div>
        </div>
        ...
        <div className="mt-14 grid gap-3 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4">
          {filteredModules.map((key, index) => (
            <motion.article
              ...
              whileHover={{ y: -4 }}
              className="group rounded-3xl border border-[#082f3a]/[0.07] bg-white p-5 transition-shadow duration-300 hover:shadow-[0_20px_50px_rgba(8,47,58,0.07)]"
            >
              ...
              <h3 className="mt-7 text-sm font-semibold tracking-tight text-[#082f3a]">
                {t(`items.${key}.title`)}
              </h3>
        ...
        {filteredModules.length === 0 && (
          <div className="mt-12 rounded-[28px] border border-dashed border-[#082f3a]/10 bg-white p-12 text-center">
            <p className="text-sm text-[#71838a]">
              {t("empty")}
            </p>
          </div>
        )}
